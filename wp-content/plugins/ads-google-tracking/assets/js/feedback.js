jQuery(document).ready(function($) {
	$('#ip2location-country-blocker-feedback-modal').dialog({
		...
		open: function() {
			$('.ui-widget-overlay').bind('click', function() {
				$('#ip2location-country-blocker-feedback-modal').dialog('close');
			});
		},
	...
	$('.deactivate a').each(function(i, ele) {
		if ($(ele).attr('href').search('ip2location-country-blocker') > -1) {
			...
			$(ele).on('click', function(e) {
				...
				$('#ip2location-country-blocker-feedback-modal').dialog('open');
			});
